...
exports.GetAllSubcategoriByID = async (req, res) => {
  try {
    ...
    if (!subcategoryId) {
      return res
        .status(400)
        .json({ message: "SubCategory ID is missing from the request" });
    }

    const subcategory = await SubCategorie.findById(subcategoryId).populate(
      "categorie_id"
    );
...
exports.Deletesub = async (req, res) => {
  ...
  try {
    ...
    if (productsLength > 0) {
      return res.status(400).json({
        message: "SubCategory has associated Products. Delete Products first.",
      });
    }
